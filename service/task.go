...
func (s *taskService) UpdateTask(ctx context.Context, task *entity.Task) (entity.Task, error) {
	if task.CategoryID != 0 {
		...
		result, _ :=s.taskRepo.GetTaskByID(ctx, task.ID)

		temp := task.CategoryID - result.CategoryID
		
		if temp > 0 {
			cat, _ =s.categoryRepo.NextCategory(result.CategoryID)
			task.CategoryID = cat.ID
		} else {
			cat, _ =s.categoryRepo.PreviousCategory(result.CategoryID)
			task.CategoryID = cat.ID
		}
